...
class DashboardLaboratorioHub extends StatefulWidget {
  ...
  State<DashboardLaboratorioHub> createState() =>
      _DashboardLaboratorioHubState();
}

class _DashboardLaboratorioHubState extends State<DashboardLaboratorioHub> {
  ...
  Future<void> _carregarIconesCustomizados() async {
    try {
      ...
      debugPrint("Aviso: Imagens não encontradas. Erro: $e");
    }
  }
  ...
  Widget _buildKpiCard({
    ...
    return Material(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(12),
        onTap: () =>
            setState(() => _filtroAtual = isSelecionado ? "Todas" : filtroAlvo),
        child: Padding(
          padding: const EdgeInsets.all(16.0),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildFilterChip(String label, IconData icone, [Color? corDestaque]) {
    ...
    return FilterChip(
      ...
      onSelected: (bool selected) =>
          setState(() => _filtroAtual = selected ? label : "Todas"),
    );
  }
