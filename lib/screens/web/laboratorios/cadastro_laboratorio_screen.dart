...
class ListaLaboratoriosView extends StatefulWidget {
  final ValueChanged<Laboratorio> onLabSelected; // 💡 GATILHO DA NOVA UX MESTRE-DETALHE
...
class _ListaLaboratoriosViewState extends State<ListaLaboratoriosView> {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView( // 💡 Proteção principal contra RenderFlex (Tela Vermelha)
      child: Padding(
                ...
                filtrados.sort((a, b) {
                  ...
                  switch (_sortColumnIndex) {
                    ...
                    case 1:
                      ...
                      break;
                    case 2:
                      result = a.endereco.cidade.toLowerCase().compareTo(b.endereco.cidade.toLowerCase());
                      break;
                    default:
                    ...
                    Container(
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: DataTable(
                          ...
                          columns: [
                            DataColumn(
                              label: const Text('Laboratório', style: TextStyle(fontWeight: FontWeight.bold)),
                              onSort: (col, asc) => setState(() { _sortColumnIndex = col; _isAscending = asc; }),
                            ),
                            const DataColumn(
                              label: Text('CNPJ', style: TextStyle(fontWeight: FontWeight.bold)),
                            ),
                            DataColumn(
                              label: const Text('Localização', style: TextStyle(fontWeight: FontWeight.bold)),
                              onSort: (col, asc) => setState(() { _sortColumnIndex = col; _isAscending = asc; }),
                            ),
                            const DataColumn(
                          ...
                          rows: paginados.map((lab) => _buildRowReal(lab)).toList(),
                        ),
                      ),
                    ),
                            ...
                            child: Row(
                              children: [
                                IconButton(
                                  ...
                                  color: _currentPage > 0 ? Colors.indigo : Colors.grey.shade300,
                                  onPressed: _currentPage > 0 ? () => setState(() => _currentPage--) : null,
                                ),
                                Container(width: 1, height: 24, color: Colors.grey.shade300),
                                IconButton(
                                  icon: const Icon(Icons.chevron_right_rounded),
                                  color: _currentPage < totalPages - 1 ? Colors.indigo : Colors.grey.shade300,
  ...
  DataRow _buildRowReal(Laboratorio lab) {
    return DataRow(
      cells: [
        DataCell(
          InkWell(
            onTap: () => widget.onLabSelected(lab), // Clicou no nome? Entra no laboratório!
  ...
  void _abrirModalFormulario(BuildContext context, {Laboratorio? labEdicao}) {
    ...
    final telefoneController = TextEditingController(text: labEdicao?.telefone ?? '');

    final cepController = TextEditingController(text: labEdicao?.endereco.cep ?? '');
    final ruaController = TextEditingController(text: labEdicao?.endereco.logradouro ?? '');
    final numeroController = TextEditingController(text: labEdicao?.endereco.numero ?? '');
    final cidadeController = TextEditingController(text: labEdicao?.endereco.cidade ?? '');
    final estadoController = TextEditingController(text: labEdicao?.endereco.estado ?? '');
